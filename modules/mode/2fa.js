...
class Twofa {
  ...
  async submitverify(selector) {
    ...
    try {
      ...
      if (attr === null) {
        ...
        manager.emit(this.STATE_EVENTS.CHANGE_STATUS, this.STATE.STOP_BOT);
      }
    } catch (err) {
      manager.emit(this.STATE_EVENTS.CHANGE_STATUS, this.STATE.OK);
    }
  ...
  async start_twofa_location_check() {
    ...
    try {
      ...
      if (attr === null) {
        ...
        manager.emit(this.STATE_EVENTS.CHANGE_STATUS, this.STATE.OK_NEXT_VERIFY);
      }
    } catch (err) {
      manager.emit(this.STATE_EVENTS.CHANGE_STATUS, this.STATE.ERROR);
    }
    ...
    this.log.info(`status location: ${manager.get_status()}`);
  }
  ...
  async start_twofa_check() {
    ...
    await this.utils.sleep(this.utils.random_interval(3, 6));
    this.log.info(`status: ${manager.get_status()}`);
  }
